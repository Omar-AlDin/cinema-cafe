...
const fetchMovies = async function (page = 1) {
  ...
  try {
    ...
    const effectiveMinRating =
      currentFilters.minRating || (isArabic ? "5.5" : "");
...
if (movieGrid) {
  if (isBackForwardNavigation()) {
    ...
    if (saved) {
      ...
      if (currentPage >= pagePause && loadButton)
...
const genresMovies = async function () {
  ...
  try {
    ...
    if (!genreRes.ok)
      throw new Error(`HTTP response failed: ${genreRes.status}`);
...
const observerTv = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) checkSentinelTv();
  });
}, observerOptions);
...
const tvImage = async function (page = 1) {
  ...
  const signal = currentFetchControllerTv
    ? currentFetchControllerTv.signal
    : undefined;
...
const genresTv = async function () {
  ...
  try {
    ...
    const genreData = await genreRes.json();
    if (!genreRes.ok)
      throw new Error(`HTTP response went wrong: ${genreRes.status}`);
    // console.log("Genre Data", genreData)
    genreData.genres.forEach((genre) => {
      const option = document.createElement("option");
      option.value = genre.id;
...
document
  .querySelector("#tv-rating-select")
  ?.addEventListener("change", function (e) {
...
document
  .querySelector("#tv-language-select")
  ?.addEventListener("change", function (e) {
...
const renderDetails = function (movie) {
  ...
  //Certificate
  const relDates = movie.release_dates?.results?.find(
    (item) => item.iso_3166_1 === movie.origin_country[0],
  );
  const certificate = relDates?.release_dates?.find(
    (cer) => cer.certification != "",
  )?.certification;
  ...
  if (!movieReleaseDate) {
    document.querySelector("#tab-title").textContent =
      `${movie.title} | Cinema Cafe`;
  } else {
    document.querySelector("#tab-title").textContent =
      `${movie.title} (${movie.release_date.split("-")[0]}) | Cinema Cafe`;
  }
  ...
  if (actorsContainer) {
    ...
    actors.forEach((act) => {
      ...
      attore.innerHTML = `<img src=${profileImage} alt="${act.name}" class = "cast-image">
...
<h4 class="actor-name">${act.name}</h4>
  ...
  //Writers
  const writing =
    movie.credits?.crew?.filter((dep) => dep.department === "Writing") || [];

  const uniqueNames = writing?.filter(
  ...
  //Directors
  const director =
    movie.credits?.crew?.filter((d) => d.job === "Director") || [];
...
const renderDetailsTv = function (tv) {
  ...
  document
    ...
    .forEach((el) => (el.src = imageUrl(tv.poster_path)));
  showAll(".poster");
  ...
  if (!tvDate) {
    ...
    document.querySelector("#tab-title").textContent =
      `${tv.name} (${tv.first_air_date.split("-")[0]}) | Cinema Cafe`;
  }
  ...
  const certification =
    rating?.find((c) => c.iso_3166_1 === tv.origin_country[0]) ||
    rating?.find((c) => c.iso_3166_1 === "US") ||
    rating[0];
  ...
  if (!runTimes) {
    runTimes =
      tv.last_episode_to_air?.runtime || tv.next_episode_to_air?.runtime;
  }
  ...
  if (releaseDate <= today) {
    ...
    setAllText(
      ".origin-country",
      formatedCountry(tv.origin_country[0]) || "N/A",
    );
...
const renderSeasonBtn = function (tv) {
  ...
  tv.seasons.forEach((sea) => {
    ...
    if (
      firstRealSeason &&
      sea.season_number === firstRealSeason.season_number
    ) {
...
const renderEpisode = function (seasonData) {
  ...
  episodes.forEach((ep) => {
    ...
    const runtimeText = ep.runtime
      ? `<span class="dotta">•</span> ${ep.runtime}m`
      : "";

    seasonEpi.insertAdjacentHTML(
      "beforeend",
      `
...
const spans = function (movie) {
  document
    .querySelectorAll(".poster")
    .forEach((el) => (el.src = imageUrl(movie.poster_path)));
  document
    .querySelectorAll(".poster")
    .forEach((el) => (el.alt = `${movie.original_title} Poster`));
  ...
  if (releaseDate <= today) {
    ...
    setAllText(
      ".origin-country",
      formatedCountry(movie.origin_country[0]) || "N/A",
    );
  } else {
    setAllText(".date-label", "Coming Soon");
    setAllText(".date-span", formatedDateOnly(movie.release_date) || "N/A");
...
if (savedSearch && searchInput) {
  ...
  if (currentSearchPage >= searchPagePause && searchLoadBtn)
    searchLoadBtn.style.display = "block";
...
const fetchSearch = async function (query, page = 1) {
  ...
  try {
    ...
    if (!searchRes.ok)
      throw new Error(`HTTP response error ${searchRes.status}`);
  ...
  try {
    if (isTv) {
      ...
      if (backdropImage) {
        ...
        document
          .querySelector(".ambient-glow")
          .style.setProperty("--glow-image", `url(${images.backdrop})`);
      }
      const mediaOk = await mediaAsset(movieId);
      if (mediaOk) {
        renderPopular();
  ...
  if (trailerButton) {
    trailerButton.forEach((el) => {
      el.addEventListener("click", function () {
        const mainTrailer =
          mediaState.videos.find(
            (trail) =>
              trail.site === "YouTube" &&
              trail.name === "Official Trailer" &&
              trail.official === true,
          ) ||
          mediaState.videos.find(
            (trail) =>
          ...
          mediaState.videos.find(
            (trail) => trail.site === "YouTube" && trail.type === "Teaser",
          );
        ...
        if (isTv) {
          setAllText(
            ".trailer-name",
            `${Itemname.name} - ${mainTrailer.type || "Trailer"}`,
          );
        } else {
          setAllText(
            ".trailer-name",
            `${Itemname.title} - ${mainTrailer.type || "Trailer"}`,
          );
        }
      });
    });
...
const truncateText = function (text, maxLength = 104) {
  ...
  return text.length > maxLength
    ? text.slice(0, subString.lastIndexOf(" ")).trim() + "..."
    : text;
};
...
const renderHero = async function (item) {
  ...
  const logoObj =
    data.images?.logos?.find((logo) => logo.iso_639_1 === "en") ||
    data.images?.logos?.[0];
  ...
  if (isTv) {
    ...
    const cert =
      ratings.find(
        (c) => c.iso_3166_1 === (data.origin_country?.[0] || "US"),
      ) || ratings[0];
    ...
    certificate = relDates?.release_dates?.find(
      (c) => c.certification !== "",
    )?.certification;
  }
...
const fetchHomepageBackdrops = async function () {
  ...
  try {
    ...
    if (!backdropRes.ok)
      throw new Error(`HTTP response went wrong: ${backdropRes.status}`);
  ...
  heroTrailerBtn.addEventListener("click", () => {
    ...
    const mainTrailer =
      videoList?.find(
        (trail) =>
          trail.site === "YouTube" &&
          trail.name.toLowerCase().includes("official trailer") &&
          trail.official === true,
      ) ||
      videoList.find(
        (trail) =>
    ...
    if (currentItem.media_type === "tv") {
      document.querySelector(".trailer-name").textContent =
        `${currentItem.data.name} - ${mainTrailer.type || "Trailer"}`;
    } else {
      document.querySelector(".trailer-name").textContent =
        `${currentItem.data.title} - ${mainTrailer.type || "Trailer"}`;
    }
  });
}
...
const renderPopularHome = async function (mediaType = "movie") {
  ...
  try {
    ...
    if (!urlRes.ok)
      throw new Error(`HTTP response went wrong: ${urlRes.status}`);
...
const renderHighlyRated = async function () {
  ...
  try {
    ...
    if (!movieRes.ok)
      throw new Error(`HTTP response went wrong: ${movieRes.status}`);
    if (!tvRes.ok) throw new Error(`HTTP response went wrong: ${tvRes.status}`);
...
const renderGems = async function () {
  ...
  try {
    ...
    const request = hiddenGems.map(async (item) => {
      ...
      if (!res.ok)
        throw new Error(
          `Failed to fetch ${item.type} (ID: ${item.id}) - Status: ${res.status}`,
        );
    ...
    gems.forEach((item) => {
      const detailsPage = item.first_air_date
        ? "tv-details.html"
        : "movie-details.html";
      const image = document.createElement("img");
      image.alt = item.first_air_date ? item.name : item.title;
